    ...
    <div class="w-[20rem] h-[22rem] mt-4 ml-4 relative">
      ...
      <div class="w-full left-[15px] top-[12rem] absolute flex space-x-2">
        <div class="w-[8.5rem]">
          ...
          <input type="text" id="pressure" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1').replace(/^0[^.]/, '0');" min="0" maxlength="4" class="w-full p-1 bg-gray-100 rounded-[10px]" />
        </div>
        <div class="w-[8.5rem]">
          <label for="wind-average" class="text-white text-sm font-medium font-poppins">Wind Average</label>
          <input type="text" id="meanWind" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1').replace(/^0[^.]/, '0');" min="0" maxlength="3" class="w-full p-1 bg-gray-100 rounded-[10px]" />
        </div>
      </div>
  ...
  <div class="z-50 w-full h-full">
    ...
    <div :class="{ 'hidden': !showTrackList }" class="w-[50rem] h-[18rem] bg-dark rounded-[10px] fixed bottom-4 right-4">
      ...
      <div class="max-h-[15rem] overflow-auto">
        ...
        <tbody>
          <tr v-for="(track, index) in tracks.list" :key="index">
...
const submitData = async () => {
  try {
    ...
    const payload = {
      ...
      forecast: disturbanceData.forecast, 
    ...
    if (tracks.list.length > 0) {
      ...
      if (isConfirmed) {
        ...
        const response = await axios.post(
          "https://tropicalcyclone.bmkg.go.id/api-tcwc/tcwc/cyclone/create",
          payload
        );
